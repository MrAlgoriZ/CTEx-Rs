...
impl CycleManager {
    ...
    //     if self.tasks.read().await.contains_key(&symbol) {
    //         return Err(format!("Цикл {} уже существует", symbol));
    //     }
    ...
    //     self.spawn_cycle_with_restart(symbol, &cycle_type, &model, Arc::clone(&self.counters))
    //         .await;
