...
> Важно: если вы хотите сохранить именно `candles`, то это название таблицы нужно поменять в SQL-запросах в `src/data/requests/database/requests.rs`.

---

Теперь у вас есть чёткая схема для PostgreSQL и список колонок, необходимых для работы CTEx-Rs.